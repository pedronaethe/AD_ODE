...
function get_model_4vel(X::Vector{Float64})
    """
    ...
    l = l0/(1 + R) * R^(1.5)
    bl_gcov::Array{Float64, 2} = gcov_bl(r, th)
    bl_gcon::Array{Float64, 2} = gcon_func(bl_gcov)
    gcov::Array{Float64, 2} = gcov_func(X)
    gcon::Array{Float64, 2} = gcon_func(gcov)

    bl_Ucov::Vector{Float64} = zeros(Float64, NDIM)
    ...
    bl_Ucov[3] = 0.0
    bl_Ucov[4] = l * ubar
    bl_Ucon::Vector{Float64} = flip_index(bl_Ucov, bl_gcon)

    ks_Ucon::Vector{Float64} = bl_to_ks(X, bl_Ucon)
    Ucon::Vector{Float64} = vec_from_ks(X, ks_Ucon)
    Ucov::Vector{Float64} = flip_index(Ucon, gcov)
    Bcon::Vector{Float64} = zeros(Float64, NDIM)
    Bcon[3] = 1.0
    Bcov::Vector{Float64} = flip_index(Bcon, gcov)

    return Ucon, Ucov, Bcon, Bcov
end


function get_model_ne(X::Vector{Float64})
    """
